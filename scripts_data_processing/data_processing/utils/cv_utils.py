...
def parse_pinhole_intrinsics(json_data: dict) -> Dict[str, np.ndarray]:
    ...
    # pinhole parameters
    fx = intr_data['focal_length_x']
    fy = intr_data['focal_length_y']
    px = intr_data['principal_pt_x']
    py = intr_data['principal_pt_y']
    
    dist = intr_data['radial_distortion']
...
def get_pruner_z(tag_dict, left_id, right_id, nominal_z=0.072, z_tolerance=0.008):
    left_z = None
    if left_id in tag_dict:
        tvec = tag_dict[left_id]['tvec']
        # check if depth is reasonable (to filter outliers)
